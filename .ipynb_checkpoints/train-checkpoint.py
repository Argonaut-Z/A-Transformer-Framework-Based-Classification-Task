...
def train_model(config):
    ...
    for epoch in range(config.epochs):
        ...
        for idx, (sample, label) in enumerate(train_iter):
            ...
            padding_mask = (sample == data_loader.PAD_IDX).transpose(0, 1)  # [batch_size, src_len]
            """
            要去生成一个padding_mask矩阵
            encoder不需要attention_mask，所以默认为None就可以了
            """
            ...
            acc = (logits.argmax(1) == label).float().mean()        # 计算准确率
            if idx % 10 == 0:
